...
{
	unsigned addr = (cpu->r.C << 2) + cpu->r.L + count;
...
* Note: this is OK, since A never contains modified data in character mode
* VERIFIED against Paul's JavaScript 17-10-16
***********************************************************************/
void jumpOutOfLoop(CPU *cpu, int count)
{
        ADDR15  temp = cpu->r.S; // save S (not the way the hardware did it)
...
int exitSubroutine(CPU *cpu, int in_line)
{
        ...
        if (OPERAND(cpu->r.B)) {
                ...
                result = 2;
                if (cpu->r.NCSF) {
			causeSyllableIrq(cpu, IRQ_FLAG, "XIT no FLAG");
                }
        } else {
                // flag bit is set
